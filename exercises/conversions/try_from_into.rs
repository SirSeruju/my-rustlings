...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        let mut ss = slice.iter();
        Ok(Color {
            red: match (*ss.next().expect("unreachable")).try_into() {
                Ok(x) => x,
                Err(_) => return Err(IntoColorError::IntConversion)
            },
            green: match (*ss.next().expect("unreachable")).try_into() {
                Ok(x) => x,
                Err(_) => return Err(IntoColorError::IntConversion)
            },
            blue: match (*ss.next().expect("unreachable")).try_into() {
                Ok(x) => x,
                Err(_) => return Err(IntoColorError::IntConversion)
            }
        })
